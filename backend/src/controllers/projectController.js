...
const updateProject = async (request, reply) => {
  try {
    ...
    // Only the student or supervisor assigned can update
    if (
      user.role !== 'admin' &&
      project.student_id !== user.id &&
      project.supervisor_id !== user.id
...
const addAttachment = async (request, reply) => {
  try {
    ...
    // Only the student or supervisor assigned can add attachments
    if (
      user.role !== 'admin' &&
      project.student_id !== user.id &&
      project.supervisor_id !== user.id
...
const deleteAttachment = async (request, reply) => {
  try {
    ...
    // Only the student or supervisor assigned can delete attachments
    if (
      user.role !== 'admin' &&
      project.student_id !== user.id &&
      project.supervisor_id !== user.id
...
// Create a project from an approved proposal (internal use)
